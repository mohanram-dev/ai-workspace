...
import { approveRequestSchema } from "@aiw/shared";
...
/** POST /api/approvals/:id/approve — `{scope: "once" | "task"}`; the waiting task continues. */
export async function POST(request: Request, ctx: RouteContext<"/api/approvals/[id]/approve">): Promise<Response> {
  try {
    ...
    const { scope } = await readJson(request, approveRequestSchema);
    ...
    const decided = await getAgentServices().approvals.decide(id, { status: "approved", scope, decidedBy: user.id });
    ...
    await writeAuditLog(db, {
      ...
      resourceId: id,
      metadata: { tool: decided.toolName, scope, taskId: decided.taskId, action: decided.action },
    });
